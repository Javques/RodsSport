...
static const float gridLineX[] = {
        -50.0f, 0.0f, 0.0f, 0.5f, 0.5f, 0.5f,
        50.0f, 0.0f, 0.0f, 0.5f, 0.5f, 0.5f
};

static const float gridLineZ[] = {
        0.0f, 0.0f, 50.0f, 0.5f, 0.5f, 0.5f,
        0.0f, 0.0f, -50.0f, 0.5f, 0.5f, 0.5f
};
...
void drawXGrid(Shader shader)
{
    std::string matName = "model";

    for (int i = -50; i < 50; i++) {
        glm::mat4 model = glm::mat4(1.0f);
        model = rotateWorld(model);
        model = glm::translate(model, glm::vec3(0, -1, i));
        shader.setMat4(matName.c_str(), model);
        glDrawArrays(GL_LINES, 0, 2);
    }
}

void drawZGrid(Shader shader)
{
    std::string matName = "model";
