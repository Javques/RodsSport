...
// move to a point, look at a point
void Camera::moveAndLookAt(glm::vec3 pos, glm::vec3 lookAt)
{
    camera_position = pos;
    camera_front = glm::normalize(pos - lookAt);
    euler_angle.x = -90;
    euler_angle.y = 0;
    update();
}
...
void Camera::mouseLook(float xoffset, float yoffset)
{
    ...
    // ??????
    camera_orientation = glm::rotate(camera_orientation, xoffset, glm::vec3(0, 1, 0));
    camera_orientation = glm::rotate(camera_orientation, yoffset, glm::vec3(1, 0, 0));
